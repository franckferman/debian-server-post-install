...
Automates system configuration, hardening, and tooling setup after a fresh Debian-based server install.
...
Automates system configuration, hardening, and tooling setup for Debian-based servers.
...
check_root() {
    # Check if script is run as root (not allowed)
    if [[ $EUID -eq 0 ]]; then
...
# Authentication
PermitRootLogin $(if $DISABLE_ROOT_SSH; then echo "no"; else echo "yes"; fi)
PasswordAuthentication $(if $SSH_KEY_ONLY; then echo "no"; else echo "yes"; fi)
